...
class Book(db.Model):
    ...
    print_length = db.Column(db.String(100))
    release_date = db.Column(db.String(100))
    edition = db.Column(db.String(100))
    format = db.Column(db.String(100))
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self, title, author, isbn, print_length, release_date, edition,format, user_token, id =''):
        self.id = self.set_id()
        self.title = title
        self.author = author
        self.isbn = isbn
...
class BookSchema(ma.Schema):
    class Meta:
        fields = ['id', 'title', 'author', 'isbn', 'print_length', 'release_date', 'edition', 'format']
